...
from lib import Resnet, Resnet_torch


def resnet50_stride32():
    return DepthNet(backbone='resnet', depth=50, upfactors=[2, 2, 2, 2])

def resnet101_stride32x8d():
    return DepthNet(backbone='resnet101_32x8d', depth=101, upfactors=[2, 2, 2, 2])
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
        ...
        elif self.backbone == 'resnet101_32x8d':
            self.encoder = Resnet_torch.resnet101_32x8d(pretrained=self.pretrained)
        else:
            self.encoder = Resnet_torch.resnet101(pretrained=self.pretrained)
